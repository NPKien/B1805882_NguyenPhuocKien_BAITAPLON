  ...
  <div class="addressa">
    ...
    <ul class="book-list">
      <li v-for="book in books" :key="book._id" class="book-item">
        <p class="book-info"> <span class="book-title">{{ book.TENSACH }}</span> <span class="author">Tác giả: {{ book.TACGIA }}</span> </p>
        <button @click="deleteBook(book._id)" class="delete-button">Xóa</button>
        <router-link :to="'/edit/' + book._id" class="edit-link">Chỉnh sửa</router-link>
      </li>
    </ul>
  </div>
</template>
  ...
  methods: {
    ...
    async deleteBook(bookId) {
      try {
        ...
        this.fetchBooks(); // Làm mới danh sách sách sau khi xóa
      } catch (error) {
...
.book-title {
  color: #007bff; /* Màu cho tên sách */
}

.author {
  color: #28a745; /* Màu cho tên tác giả */
}
.search-container {
  margin-bottom: 20px;
}
